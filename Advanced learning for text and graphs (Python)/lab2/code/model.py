...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        embedded = self.embedding(input)
        res,h = self.rnn(embedded,h)
        concat = self.ff_concat(torch.cat((source_context,h),dim=2))
        th = torch.tanh(concat)
        prediction = self.predict(th)
        return prediction, h
