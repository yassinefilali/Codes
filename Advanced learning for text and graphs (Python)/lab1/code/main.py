...
def compute_loss(prodpos,prodnegs):
    ...
    ### fill the gaps ###
    term_pos = np.log([1+np.exp(-x) for x in prodpos])
    term_negs = np.log([1+np.exp(x) for x in prodnegs])
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    ### fill the gaps ### 
    partials_pos = factors_pos.reshape(-1,1) @ -Wt[target,].reshape(1,-1)
    partials_negs =  factors_negs.reshape(-1,1) @ Wt[target,].reshape(1,-1)
    term_pos = - Wc[pos,] * factors_pos.reshape(-1,1)
...
path_read = "C://Users//Yassine//Desktop//ALTEGRAD//LAB1//data//"
...
if not train:
    ...
    print("cosimilarity for movie and film",my_cos_similarity('movie','film'))
    print("cosimilarity for movie and banana",my_cos_similarity('movie','banana'))
    print("cosimilarity for night and sleep",my_cos_similarity('night','sleep'))
    print("cosimilarity for night and disk",my_cos_similarity('night','disk'))
